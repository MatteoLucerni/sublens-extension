...
- `autoPauseOnHover`: hovering a subtitle pauses the video (`pauseForInteraction`, which remembers whether it was playing in `wasPlayingBeforePause`); ending the interaction resumes it through `releaseInteraction`. The outside-click handlers (`onOutsideClick`, `onSelectionOutsideClick`, document capture phase) call `consumePlayerClick` first: a click inside the visible video area (`isClickInsidePlayer`, using `getVisibleVideoRect`) gets `preventDefault` + `stopPropagation`, so it only dismisses the popup/selection. Otherwise the immediate resume in `releaseInteraction` would be followed by the player's own click-to-toggle handler pausing the video again. Players that toggle on `mousedown`/`pointerup` have already resumed before the click, so the resume is a no-op for them. Clicks outside the video pass through unchanged.
